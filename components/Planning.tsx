...
export const Planning: React.FC<Props> = ({ 
  ...
  onAddGoal, onDeleteGoal, onUpdateGoal, onAddBudget, onDeleteBudget, lang 
}) => {
  ...
  return (
    <div className="space-y-8 pb-24 md:pb-0">
      ...
      <div>
        <div className="flex justify-between items-center mb-4">
           <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
             <Target className="text-indigo-600" /> {t.planning.goals}
           </h2>
           <button 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {goals.map(goal => {
            ...
            return (
              <div key={goal.id} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
                 <div className="flex justify-between items-start mb-3">
                    <div className="p-3 bg-indigo-50 text-indigo-600 rounded-xl">
                      {getIcon(goal.icon)}
                    </div>
                    <button onClick={() => onDeleteGoal(goal.id)} className="text-slate-300 hover:text-rose-500"><Trash2 size={16}/></button>
                 </div>
          ...
          {goals.length === 0 && (
            <div className="col-span-full py-10 text-center border-2 border-dashed border-slate-200 rounded-2xl text-slate-400">
      ...
      <div>
        <div className="flex justify-between items-center mb-4">
           <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
             <TrendingUp className="text-indigo-600" /> {t.planning.budgets}
           </h2>
           <button 
             onClick={() => setBudgetModalOpen(true)}
             className="bg-white border border-slate-200 text-slate-700 px-3 py-2 rounded-lg text-sm font-medium hover:bg-slate-50 flex items-center gap-1"
           >
                   ...
                   <div key={budget.id} className="p-4 hover:bg-slate-50 transition-colors">
                      <div className="flex justify-between items-center mb-2">
                         <div className="flex items-center gap-3">
                            <span className="font-semibold text-slate-700">{transCategory}</span>
      ...
      {isGoalModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm p-6 animate-in fade-in zoom-in duration-200">
            <h3 className="text-lg font-bold mb-4">{t.planning.modalGoalTitle}</h3>
            <div className="space-y-3">
               ...
               <div className="flex gap-2 justify-center py-2">
                  {['plane', 'home', 'card', 'shopping', 'target'].map(ic => (
                    <button 
                      ...
                      className={`p-2 rounded-lg ${goalForm.icon === ic ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-50 text-slate-400'}`}
                    >
                      {getIcon(ic)}
                    </button>
                  ))}
               ...
               <div className="flex gap-2 mt-4">
                 <button onClick={() => setGoalModalOpen(false)} className="flex-1 py-2 text-slate-500 hover:bg-slate-50 rounded-lg">{t.manual.cancel}</button>
      ...
      {isBudgetModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm p-6 animate-in fade-in zoom-in duration-200">
            <h3 className="text-lg font-bold mb-4">{t.planning.modalBudgetTitle}</h3>
            <div className="space-y-3">
               <select
                 className="w-full p-2 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500"
               ...
               />
               <div className="flex gap-2 mt-4">
                 <button onClick={() => setBudgetModalOpen(false)} className="flex-1 py-2 text-slate-500 hover:bg-slate-50 rounded-lg">{t.manual.cancel}</button>
